...
Node::Node(const UFNode& player) : data(new UFNode(player)), next(nullptr) {}

Node::Node(UFNode* player) : data(player), next(nullptr){}

Node::Node(const Node& node)
{
    data = new UFNode(*(node.data));
    if (node.next) {
...
void LinkedList::insert(const UFNode& player)
{
...
void LinkedList::insertWithPtr(UFNode* player)
{
    Node* node_to_add = new Node(player);
    if (head == nullptr) {
        head = node_to_add;
