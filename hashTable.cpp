...
void HashTable::resize(int new_size, LinkedList* old_players_array, int old_size)
{
    ...
    {
        UFNode* currentPlayer = currentNode->data;
        currentNode->data = nullptr;
...
UFNode* HashTable::find(int player)
{
...
HashTableStatus HashTable::insert(UFNode* player)
{
    if (counter == size / 2) {
...
HashTableStatus HashTable::remove(UFNode& player)
{
    if (counter == size / 4) {
